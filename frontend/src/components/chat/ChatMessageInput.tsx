...
type ChatMessageInput = {
  ...
  externalMessage?: string | null; // NEW: Allow external injection of message
  onExternalMessageConsumed?: () => void; // NEW: Callback when external message is consumed
};
...
export const ChatMessageInput = ({
  ...
  // NEW: Handle external message injection (from code editor)
  useEffect(() => {
    if (externalMessage && externalMessage.trim() !== "") {
      ...
      // Notify parent that message was consumed
      if (onExternalMessageConsumed) {
        onExternalMessageConsumed();
      }
      
      // AUTO-SEND the message after a brief delay (so user can see it)
      setTimeout(() => {
        if (onSend && externalMessage.trim() !== "") {
          onSend(externalMessage);
          setMessage("");
        }
      }, 500);
    }
  }, [externalMessage, onSend, onExternalMessageConsumed]);
  ...
  return (
    <div
      className="flex flex-col gap-2 border-t border-t-gray-300 dark:border-t-gray-800"
      style={{ height: height }}
    >
      <div className="flex flex-row pt-3 gap-2 items-center relative">
        ...
        <input
          ref={inputRef}
          className={`w-full text-xs caret-transparent bg-transparent opacity-25 text-gray-900 dark:text-gray-300 p-2 pr-6 rounded-sm focus:opacity-100 focus:outline-none focus:border-${accentColor}-700 focus:ring-1 focus:ring-${accentColor}-700`}
          style={{
